...
**Production is live at product commit `586d818`.** Cloudflare version `49f153cb-61ac-4584-aa8d-79323fa85e73` serves that build at 100% on <https://atlas-news-intelligence-api.atlas-news-surface.workers.dev>, backed by the existing `atlas-news-intelligence-prod` D1 database. The later event-ecosystem ledger is documentation-only; no deployment was performed for it. Immersive Commons accepted the separately approved Atlas submission at `2026-08-27T22:59:39.587Z`; a post-lock read verified event phase `LOCKED` and `submission.locked: true`. The [sanitized submission receipt](docs/HACKATHON_SUBMISSION_2026-08-27.md) preserves the exact public payload and status. Fresh production Ora/IsItAgentReady scans and an exact-version Agent D pass remain documented limitations rather than retroactively claimed passes. Runtype's existing A2A capability genuinely executes against production, but all three Runtype surfaces remain draft because Runtype returned HTTP 403 before any of the five named eval suites could be created or run. Eval usage remained 0/100 and no personal provider billing was attached.

Atlas genuinely used the event ecosystem across different phases: Immersive Commons, Cloudflare, Cotal, Nebius, Tenki, Tavily, Mitosis Labs, Runtype, and Hacker Bob. The [event-ecosystem usage ledger](docs/SPONSOR_USAGE.md) distinguishes production integrations, build-time workforce/tool use, authenticated provider exercise, and truthful limitations; it also records that AIsa and HUD were deliberately unused. Exact deployment and D1 evidence remains available in the [production release receipt](docs/PRODUCTION_RELEASE_2026-08-27.md).
